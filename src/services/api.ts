 ...
 * (see `.env.local`). Defaults to `http://127.0.0.1:8000` for local dev.
 *
...
export type IconMappingUpdate = Partial<{
  ...
  font_color: string;
  outline_color: string;
  // NOTE: OpenAPI's IconMappingUpdate does NOT currently include x / y.
  // Per-icon position saving from the browser needs backend confirmation.
}>;
...
async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  try {
    ...
    console.error(`[api] x ${method} ${url} - network error`, err);
  ...
  if (!res.ok) {
    ...
    console.error(`[api] x ${method} ${url} - ${res.status}`, errBody);
    throw new ApiError(`Request failed: ${method} ${path} (${res.status})`, res.status, errBody);
  }

  if ((import.meta as any).env?.DEV) {
    // eslint-disable-next-line no-console
    console.debug(`[api] ok ${method} ${url} - ${res.status}`);
  }
...
/** Prefix a backend-relative URL (e.g. `/custom_icons/foo.png`) with the API base. */
